...
class LucesDAO(DataAccessDAO):
    ...
    def get(self, id):
        with self.dbconn as conn:
            cursor = conn.cursor()
            query = f"SELECT id_luz, nombre, intensidad, id_vivienda FROM {self.database}.luces WHERE id_luz = %s"
    ...
    def get_all_data(self):
        with self.dbconn as conn:
            cursor = conn.cursor()
            query = f"SELECT id_luz, nombre, intensidad, id_vivienda FROM {self.database}.luces"
            cursor.execute(query)
            rows = cursor.fetchall()
            luces = [Luces(nombre=row[1], intensidad=row[2]) for row in rows]
            return luces

    def create(self, luz: Luces):
        with self.dbconn as conn:
            cursor = conn.cursor()
            query = f"INSERT INTO {self.database}.luces (nombre, intensidad) VALUES (%s, %s)"
            values = (luz.nombre, luz.intensidad)
            cursor.execute(query, values)
            conn.commit()

    def update(self, luz: Luces):
        with self.dbconn as conn:
            cursor = conn.cursor()
            query = f"UPDATE {self.database}.luces SET nombre = %s, intensidad = %s WHERE nombre = %s"
            values = (luz.nombre, luz.intensidad, luz.nombre)
            cursor.execute(query, values)
            conn.commit()
    ...
    def delete(self, nombre):
        with self.dbconn as conn:
            cursor = conn.cursor()
            query = f"DELETE FROM {self.database}.luces WHERE nombre = %s"
